...
- [`examples/gpu-counter/README.md`](../examples/gpu-counter/README.md) — sibling 6-beat demo proving NVIDIA GPU passthrough + cuda-checkpoint via gVisor's nvproxy. Verified on three GPU classes 2026-05-27: L40S (580.126.09; Run+Checkpoint), H100 brev `front-emerald-krill` (570.195.03; full 6-beat with CUDA buffer preserved across substrate suspend/resume), H200 NVL `bigbox-h200` (580.159.03; full 6-beat). Substrate-side PR: [`agent-substrate/substrate#96`](https://github.com/agent-substrate/substrate/pull/96) (two commits — `c358dff` + the H100/H200 demo follow-up `fca2df4`).
...
- [`#96`](https://github.com/agent-substrate/substrate/pull/96) — `ActorTemplate.spec.containers[].resources.gpu`: end-to-end NVIDIA GPU passthrough via gVisor nvproxy + cuda-checkpoint. Load-bearing PR for the gpu-counter demo; verified end-to-end 2026-05-27 on L40S (580.126.09), H100 brev (570.195.03), and H200 NVL (580.159.03). Two commits on the branch: `c358dff` (CRD + protos + atelet OCI builder + ateom-gvisor `--nvproxy` flags) and `fca2df4` (five fixes the H100/H200 demo needed — device-cgroup allow, pause-with-GpuSpec, wrapper bind via `/run/ateom-gvisor/static-files/`, external CUDA drain via `cmdDrainCUDA`, `gpuSaveRestoreFlags=nil`). Driver-side counterpart on this repo at `eabfbb7`.
...
| GPU passthrough via Substrate's CDI plumbing | ✅ **Done 2026-05-27.** Driver-side reject removed, `synthesize_template` populates `containers[0].resources.gpu` from `DriverResourceRequirements.gpu` — driver commit `eabfbb7` (was `0b46450`). Substrate side: `agent-substrate/substrate#96` (`c358dff` + `fca2df4`). Verified end-to-end on three GPU classes (L40S, H100, H200) — see [`examples/gpu-counter/README.md`](../examples/gpu-counter/README.md). |
...
6. **~~GPU passthrough.~~** ✅ **Done 2026-05-27.** Filed as [`agent-substrate/substrate#96`](https://github.com/agent-substrate/substrate/pull/96) (substrate side: CRD + protos + atelet OCI builder + ateom-gvisor `--nvproxy` flags + external CUDA drain via `runsc exec supervisor cuda-checkpoint`) + driver commit `eabfbb7` (reject removed, `synthesize_template` populates `resources.gpu`, `examples/gpu-counter/` 6-beat demo). Verified end-to-end on L40S, H100, and H200.
